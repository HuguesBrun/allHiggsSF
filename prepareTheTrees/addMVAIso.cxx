...
void addMVAIso() {
    ...
    cout << "base folder" << baseFolder << endl;
    ...
    TFile *fOut = new TFile("/tmp/hbrun/tnpZ_withMVAIso.root", "RECREATE");
    fOut->mkdir("tpTree")->cd();
    TTree *tOut = tIn->CloneTree(0);
    Float_t mvaIso; Int_t mvaIsoCut;
    Int_t mvaIsoCut_HWW2012, mvaIsoCut_HWW2012Loose;
    tOut->Branch("mvaIso", &mvaIso, "mvaIso/F");
    tOut->Branch("mvaIsoCut", &mvaIsoCut, "mvaIsoCut/I");
    tOut->Branch("mvaIsoCut_HWW2012", &mvaIsoCut_HWW2012, "mvaIsoCut_HWW2012/I");
    tOut->Branch("mvaIsoCut_HWW2012Loose", &mvaIsoCut_HWW2012Loose, "mvaIsoCut_HWW2012Loose/I");


    MuonEffectiveArea::MuonEffectiveAreaTarget effAreaTarget = MuonEffectiveArea::kMuEAFall11MC;//kMuEAData2011; // NOTE: for MVA we use 2011 also for 2012 for now.
    ...
    for (int i = 0, n = tIn->GetEntries(); i < n; ++i) {
        ...
        mvaIsoCut_HWW2012 = 0;
		if ((pt >= 20) && (abs(eta)<1.479))  mvaIsoCut_HWW2012 = (mvaIso > 0.82);
		if ((pt >= 20) && (abs(eta)>1.479) && (abs(eta)<2.4))  mvaIsoCut_HWW2012 = (mvaIso > 0.86);	
		if ((pt < 20) && (abs(eta)<1.479))  mvaIsoCut_HWW2012 = (mvaIso > 0.86);
		if ((pt < 20) && (abs(eta)>1.479) && (abs(eta)<2.4))  mvaIsoCut_HWW2012 = (mvaIso > 0.82);
		
		mvaIsoCut_HWW2012Loose = 0;
		mvaIsoCut_HWW2012Loose = (mvaIso > -0.6);
        tOut->Fill();
        //if (i > 10000) break;
        if ((i+1) % step == 0) { 
